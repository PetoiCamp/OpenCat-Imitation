...
class Cat:
    ...
    def __init__(self):
        ...
        self.angles: dict = {
            ...
            'hshoulder_l': 0,  # left shoulder horizontal joint
            'hshoulder_r': 0,  # right shoulder horizontal joint
    ...
    def control_loop(self):
        if sendCmd:
            ardSerial.send(self.goodPorts,
                           ['C', [0, 0, 127, E_RGB_ALL, E_EFFECT_FLASH], 0])

        colorState = 0
        while True:
            timer = []
            timer.append(time.perf_counter())
            change = np.linalg.norm(self.prev_angles -
                                    list(self.angles.values()))
            if self.updated and change > 3:
                ...
                if self.model == 'Nybble':
                    cmd: list = [
                        ...
                        [
                            ...
                            self.angles['neck_v'],
                            sFL,
                            self.angles['shoulder_l'],
                            sFR,
                            self.angles['shoulder_r'],
                            kFL,
                            self.angles['elbow_l'],
                            kFR,
                            self.angles['elbow_r'],
                            sBL,
                            -65 - self.angles['spine'],
                            sBR,
                            -65 + self.angles['spine'],
                            kBL,
                            -30 + self.angles['spine'] / 2,
                            kBR,
                            -30 - self.angles['spine'] / 2,
                        ], 0
                    ]
                    ...
                    cmd: list = [
                        ...
                        [
                            ...
                            self.angles['elbow_r'],
                            sBL,
                            -34 - self.angles['spine'],
                            sBR,
                            -34 + self.angles['spine'],
                            kBL,
                            85 + self.angles['spine'] / 3,
                            kBR,
                            85 - self.angles['spine'] / 3,
                        ], 0
                    ]
                ...
                if sendCmd:
                    ...
                    if self.angles['neck_v'] > -15:
                        if colorState != 1:
                            ardSerial.send(self.goodPorts, [
                                'C', [64, 0, 127, E_RGB_ALL, E_EFFECT_FLASH],
                                0.0
                            ])
                            colorState = 1
                        ...
                        if colorState != 2:
                            ardSerial.send(self.goodPorts, [
                                'C', [0, 127, 127, E_RGB_ALL, E_EFFECT_FLASH],
                                0.0
                            ])
                            colorState = 2
                    else:
                        if colorState != 3:
                            ardSerial.send(self.goodPorts, [
                                'C', [64, 127, 0, E_RGB_ALL, E_EFFECT_FLASH],
                                0.0
                            ])
                            colorState = 3
                    timer.append(time.perf_counter())
...
#                    print(timer)
    ...
    def control_cat(self, model: Model):
        ...
        if self._check_thresh(model.thr, model.nose[3], model.left_shoulder[3],
            ...
            neck_angles = self._get_neck_angle(model.left_shoulder[:3],
                                               model.right_shoulder[:3],
                                               model.nose[:3])
            if abs(neck_angles[0] -
                   self.angles['neck_h']) > smallAngleThreshold:
                self.angles['neck_h'] = neck_angles[0]
            if abs(neck_angles[0] -
                   self.angles['neck_v']) > smallAngleThreshold:
...
# update shoulder l
        if self._check_thresh(model.thr, model.left_shoulder[3],
            ...
            if abs(shoulderL -
                   self.angles['shoulder_l']) > smallAngleThreshold:
...
# update shoulder r
        if self._check_thresh(model.thr, model.right_shoulder[3],
            ...
            if abs(shoulderR -
                   self.angles['shoulder_r']) > smallAngleThreshold:
                self.angles['shoulder_r'] = shoulderR
#            timer.append(time.perf_counter())

        if self._check_thresh(model.thr, model.left_shoulder[3],
                              model.right_shoulder[3], model.left_elbow[3],
                              model.right_elbow[3]):
            # update left shoulder horizontal joint
            hshoulderL = self._vec_angle(
                (model.right_shoulder - model.left_shoulder)[:3],
                (model.left_elbow - model.left_shoulder)[:3])
            if abs(hshoulderL -
                   self.angles['hshoulder_l']) > smallAngleThreshold:
                self.angles['hshoulder_l'] = hshoulderL

            # update right shoulder horizontal joint
            hshoulderR = self._vec_angle(
                (model.left_shoulder - model.right_shoulder)[:3],
                (model.right_elbow - model.right_shoulder)[:3])
            if abs(hshoulderR -
                   self.angles['hshoulder_r']) > smallAngleThreshold:
                self.angles['hshoulder_r'] = hshoulderR
...
# update elbow r
        ...
        if self._check_thresh(model.thr, model.right_shoulder[3],
                              model.left_shoulder[3], model.right_hip[3],
                              model.left_hip[3]):
            ...
            direction = np.sign(
                np.linalg.norm((model.right_shoulder - model.right_hip)[:3]) -
                np.linalg.norm((model.left_shoulder - model.left_hip)[:3]))
            spine *= direction
...
#            print(timer)
    ...
    def _get_neck_angle(self, left_shoulder, right_shoulder, nose) -> float:
        ...
        tilt = (np.linalg.norm(mid_nose) * np.sin(tilt_angle) -
                shoulderLength * 2 / 3) / (shoulderLength / 3)
